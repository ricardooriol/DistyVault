(function () {
  ...
  async function readHtml(file) {
    ...
    try {
      ...
      const text = node?.innerText || doc.body?.innerText || '';
      return DV.utils.normalizeText(text);
    } catch {
      return DV.utils.normalizeText(html.replace(/<[^>]+>/g, ' '));
    }
  }
  ...
  async function readDocx(file) {
    try {
      ...
      return DV.utils.normalizeText(result?.value || result?.text || '');
    } catch (e) {
  ...
  async function readRtf(file) {
    try {
      ...
      return DV.utils.normalizeText(raw
  ...
  async function readImageWithOCR(file) {
    ...
    try {
      ...
      return DV.utils.normalizeText(data?.text || '');
    } finally {
  ...
  async function readPdfWithOCR(pdfFile) {
    ...
    return DV.utils.normalizeText(texts.join('\n\n'));
  }
