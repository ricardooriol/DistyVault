(function () {
  ...
  async function peekYouTubeTitle(inputUrl) {
    try {
      ...
      let res = null;
      try { res = await DV.utils.fetchWithTimeout(endpoint, { headers: { 'Accept': 'application/json' } }, 7000); } catch { }
      if (!res || !res.ok) {
        try { res = await DV.utils.fetchWithTimeout('/api/fetch?url=' + encodeURIComponent(endpoint), {}, 8000); } catch { }
      }
  ...
  /** Extract JSON data assigned to a specific window variable in YouTube HTML. */
  function extractJson(html = '', marker = '') {
    const idx = html.indexOf(marker);
    if (idx === -1) return null;
    let i = html.indexOf('{', idx);
    if (i === -1) i = html.indexOf('[', idx);
    if (i === -1) return null;
  ...
  /** Choose best caption track: prefer human EN, then EN ASR, then any human, finally first. */
  function pickBestCaptionTrack(tracks) {
    ...
    return tracks.find(t => isHuman(t) && isEn(t)) ||
  ...
  /** Convert YouTube timed text XML to plain text. */
  function parseTimedTextXml(xmlString) {
    try {
      ...
      const segs = texts.map(node => {
        ...
        html = html.replace(/\n/g, ' ').replace(/<br\s*\/?\s*>/gi, '\n');
        const text = DV.utils.normalizeText(DV.utils.decodeEntities(html));
      ...
      for (const s of segs) {
        ...
        if (gap > 2.5 && buf) { out.push(buf.trim()); buf = ''; }
        else if (buf && /[\.!?]$/.test(buf.trim())) { buf += '\n'; }
        else if (buf) { buf += ' '; }
        buf += s.text.trim();
        lastEnd = s.start + s.dur;
      }
      if (buf.trim()) out.push(buf.trim());
      return DV.utils.normalizeText(out.join('\n'));
    } catch { return ''; }
  }
  ...
  async function fetchTranscriptFromTrack(baseUrl, forceEnIfTranslatable = false) {
    ...
    const res = await DV.utils.fetchWithTimeout(proxied, {}, 12000).catch(() => null);
    ...
    try {
      ...
      if (j && Array.isArray(j.events)) {
        ...
        return DV.utils.normalizeText(parts.join('\n'));
      }
    } catch { }
    return DV.utils.normalizeText(DV.utils.decodeEntities(body));
  }
  ...
  async function extractYouTube(itemOrUrl) {
    ...
    let res = await DV.utils.fetchWithTimeout('/api/fetch?url=' + encodeURIComponent(watchUrl), {}, 15000).catch(() => null);
    if (!res || !res.ok) throw new Error('Failed to load YouTube page');
    const html = await res.text();

    let title = `YouTube Video ${id}`;
    let player = extractJson(html, 'ytInitialPlayerResponse');
    if (!player) player = extractJson(html, 'ytInitialData')?.playerResponse;

    if (player?.videoDetails?.title) title = String(player.videoDetails.title);
    else { try { const peek = await peekYouTubeTitle(inputUrl); if (peek?.title) title = peek.title; } catch { } }
    ...
    const wantEnglish = !/^en(-|$)/i.test(track?.languageCode || '') && (track?.isTranslatable);
  ...
  async function extractYouTubePlaylist(itemOrUrl) {
    ...
    const res = await DV.utils.fetchWithTimeout('/api/fetch?url=' + encodeURIComponent(url), {}, 15000).catch(() => null);
