...
class ApiClient {
    ...
    async downloadPdf(id) {
        ...
        doc.write(`<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>${safeTitle}</title>
            ...
            <div class=\"meta\">Generated by DistyVault • ${new Date().toLocaleString()}</div>
            <div class=\"content\">${item.content}</div>
            <div class=\"footer\">distyvault</div>
        </body></html>`);
        doc.close();
        // Wait for fonts/layout to settle
        await new Promise(r => setTimeout(r, 50));
        const target = doc.body;
        const opt = { scale: 2, useCORS: true, backgroundColor: '#ffffff', windowWidth: 900 };
    ...
    async bulkDownload(ids) {
        const headers = new Headers({ 'Content-Disposition': 'attachment; filename="distyvault-bulk.txt"' });
        let count = 0;
        for (const id of ids) {
            try {
        ...
        // Return a trivial response object for UI consistency
        const blob = new Blob([`Triggered ${count}/${ids.length} downloads`], { type: 'text/plain' });
        return { blob, headers, status: 200 };
    }
