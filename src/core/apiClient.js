...
class ApiClient {
    ...
    // If item was previously stopped/cancelled, clear its cancellation flag so it can run
    try { this.cancellations.delete(id); } catch {}
    ...
    try { await this.db.resetTiming(id); } catch {}
        ...
        this._runWithLimit(async () => {
            ...
            // Flip to active only when our slot actually starts; set proper startTime
    ...
    async exportKnowledgeBase(options = {}) {
        ...
        // Trigger browser download without disrupting the current page (avoid white screen on iOS/Safari)
        ...
        try {
            if (isIOS || isSafari) {
                // On Safari/iOS, download attribute is unreliable; open in a new tab to keep the app intact
                const newTab = window.open(url, '_blank');
                // Keep URL alive a bit longer to ensure the download completes
                setTimeout(() => { try { URL.revokeObjectURL(url); } catch {} }, 5000);
                return { status: 'ok', filename, openedInNewTab: !!newTab };
            } else {
                const a = document.createElement('a');
                a.href = url;
                a.download = filename;
                a.style.display = 'none';
                // Ensure we don't navigate away from the SPA
                a.target = '_self';
                document.body.appendChild(a);
                a.click();
                // Give the browser time to start the download before revoking
                setTimeout(() => { try { URL.revokeObjectURL(url); } catch {} a.remove(); }, 1500);
                return { status: 'ok', filename };
            }
        } catch (e) {
            // Fallback: just navigate to the blob URL (last resort)
            try { window.location.href = url; } catch {}
            setTimeout(() => { try { URL.revokeObjectURL(url); } catch {} }, 5000);
            return { status: 'ok', filename, fallback: true };
        }
    }
